...
from tvb.simulator.integration_loop_cython import integration_loop_cython as integration_loop_cython
...
class Simulator(core.Type):
    ...
    def configure(self):
        ...
        # in some ways you generate here pyopencl or cython code
    ...
    def __call__(self, simulation_length=None, random_state=None):
        ...
        if not code_gen:
            ...
            for step in range(self.current_step+1, self.current_step+int_steps+1):
                ...
                #import pdb; pdb.set_trace()
                #if cython or pyopencl code is available, use that instead
            ...
            stimulus = numpy.array(stimulus, dtype=numpy.float32)
            weights = numpy.array(weights, dtype=numpy.float32)
            history = numpy.array(history, dtype=numpy.float32)
            node_ids = numpy.array(node_ids, dtype=numpy.dtype("i"))
            idelays = numpy.array(idelays, dtype=numpy.dtype("i"))
            cvar = numpy.array(cvar, dtype=numpy.dtype("i"))
            model_cvar = numpy.array(self.model.cvar, dtype=numpy.dtype("i"))

            x_i = numpy.zeros((1,74,1), dtype=numpy.float32)
            delayed_state = numpy.zeros((cvar.shape[0],cvar.shape[1],cvar.shape[2],1), dtype=numpy.float32)
            coupled_input = numpy.zeros((74,1,74,1), dtype=numpy.float32)
            node_coupling = numpy.zeros((1,74,1), dtype=numpy.float32)
            #import pdb; pdb.set_trace()
            monitors_curr = [monitors for monitors in self.monitors]
            for output in integration_loop_cython(self.current_step, history, idelays, state, weights, horizon, cvar, node_ids, monitors_curr, stimulus, int_steps, model_cvar, dfun, local_coupling, x_i, delayed_state, coupled_input, node_coupling):
                yield output
