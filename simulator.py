...
from tvb.simulator.scheme_cython import scheme_cython as scheme_cython
...
class Simulator(core.Type):
    ...
    def __call__(self, simulation_length=None, random_state=None):
        ...
        for step in range(self.current_step+1, self.current_step+int_steps+1):
            ...
            node_coupling = numpy.array(node_coupling, dtype=numpy.float32)
            local_coupling = numpy.array(local_coupling, dtype=numpy.float32)
            stimulus = numpy.array(stimulus, dtype=numpy.float32)
            #DX = np.zeros((2,74,1), dtype=np.float32)
            state = scheme_cython(state, dfun, node_coupling, local_coupling, stimulus)
